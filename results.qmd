...
df_hi_wide <- df_hi_wide %>%
  mutate(housing_burden = Housing / `Income after taxes`)
...
Why is transportation such a high expense for males? 
What is the housing burden by gender (% of income spent on housing-related expenses)?
How does spending on personal care products vary by age group?
What is the distribution of spending patterns for apparel and services by gender?
What is the distribution of spending patterns for healthcare sub-categories by gender?
...
# 2. Compute medians per category ----------------------------------------
median_order <- df_spend_long |>
  group_by(category) |>
  summarise(med = median(spend, na.rm = TRUE)) |>
  arrange(desc(med))

# Reorder category factor based on **descending medians**
df_spend_long <- df_spend_long |>
  mutate(category = fct_reorder(category, spend, .fun = median, .desc = TRUE))
